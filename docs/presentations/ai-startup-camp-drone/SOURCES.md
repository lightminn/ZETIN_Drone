...
| 21, 33, 78 | 상태 텔레메트리 75개 필드, 별도 1kHz 원시 IMU | [`udp_protocol.md`](../../udp_protocol.md), [`dual_imu_cascade_pwm.ino`](../../../firmware/flight/dual_imu_cascade_pwm/dual_imu_cascade_pwm.ino) | 현행 프로토콜. 필드 66~75는 allocator/PID/I/yaw 권한 상태이고 CSV는 수신 시각까지 76열 |
...
| 45~48 | 실제 비행 스케치 직접 포함, 1ms tick hook, 독립 물리·듀얼 IMU 합성, S1 정상 수렴·부호 반전 실패·저스로틀 yaw+pitch 우선순위 | [`test_sil_attitude.cpp`](../../../tools/native_tests/test_sil_attitude.cpp), [`Arduino.h`](../../../tools/native_tests/shims/Arduino.h), [`test_sil_attitude.py`](../../../tools/test_sil_attitude.py) | 현행 32비트 host SIL. S2b에서 1300→1270µs, yaw 0.030Nm·pitch 0.025Nm 외란에 RP scale 1.0000, yaw scale 0.1828, pitch 최대 11.2583°(PID anti-windup까지 동일 scale을 받는 독립 균등축소 폐루프 46.2952°), LIMITED 뒤 824ms 관측. host 모델 증거이며 실기 증거가 아님 |
...
- 아직 확인하지 않음: 반복 가능한 안정 호버, 신뢰할 수 있는 실내 저고도
  자동착륙, 저스로틀 yaw 권한 변경의 프롭 OFF/손 파지/테더/3회 착륙 게이트,
  거리·광류 폐루프 제어, 실외 자유 비행, 경로·군집 제어. 현재 optical flow
  기반 수평 위치 보정은 없으며 조종자가 수동으로 표류를 상쇄한다.
