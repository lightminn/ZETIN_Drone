...
7. 적분 해금 확인: 일정한 yaw 외란 토크 하에서 `iTermYaw`가 실제로 누적되고
   ±`I_TERM_MAX_US`로 클램프됨 (문제 2에서 바꾼 코드 자체의 회귀 테스트)

> **SIL로는 문제 2를 폐루프로 실증할 수 없다.** `test_sil_attitude.cpp:1035`가
> 명시하듯 SIL의 yaw 반작용 토크 부호는 관례 의존이라 기존 S5도 단언 없이
> `runReport`로만 남긴다(`power_on_bench_procedure.md:175`: "SIL S5는 CW=+
> 관례에서 yaw가 발산했다"). 그 위에 "정상상태 회전율이 0으로 수렴한다"는
> 폐루프 단언을 얹으면 통과할 수 없고, 통과시키려 플랜트 부호를 고치면 벤치로
> 미뤄둔 모델링 가정을 코드가 임의로 확정해버린다.
> 따라서 SIL은 **적분기가 살아 있다는 것**까지만 단언하고, 폐루프 각속도 수치는
> `runReport`로 남긴다. **정상상태 회전이 실제로 사라지는지는 벤치 Stage D에서
> 확정한다.** 같은 이유로 자동 잠금의 폐루프 동작(정착 → 잠금)도 벤치 확인
> 항목이며, SIL은 판정 로직만 단위 테스트한다.
...
- **SIL이 yaw 폐루프를 검증하지 못한다.** 플랜트의 yaw 반작용 토크 부호가
  미해결이라(§6 참조) 자동 잠금과 정상상태 회전 제거는 벤치에서만 확인된다.
  이 두 가지는 코드 리뷰와 단위 테스트만 통과한 상태로 실기에 올라간다는 뜻이므로
  Stage C-3·D를 특히 신중히 진행한다
