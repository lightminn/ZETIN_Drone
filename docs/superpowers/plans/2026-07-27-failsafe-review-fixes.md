...
### 1-1. LPF가 충격 스파이크를 죽이지 않는가 (검산)

임계 판정은 raw가 아니라 LPF 출력에 걸리므로, 짧은 스파이크는 감쇠돼 놓칠
수 있다. α=0.03(1kHz)의 계단 응답은 n ms 후 `1 − 0.97ⁿ`이다.

접지 감속거리 s, 접지속도 v=0.4m/s일 때 스파이크 크기는 `v²/2s`, 지속시간은
`2s/v`다. **s가 작아지면 크기는 커지고 지속은 짧아져 서로 상쇄된다.**

| 접지 조건 | s | 스파이크 | 지속 | 필터 통과분 | 임계 0.25g |
|---|---|---|---|---|---|
| 잔디·유연한 다리 | 2cm | 0.41g | 100ms | 0.39g | 통과 |
| 중간 | 5mm | 1.63g | 25ms | 0.83g | 통과 |
| 콘크리트·강성 다리 | 2mm | 4.08g | 10ms | 1.07g | 통과 |

접지 강성 전 구간에서 여유 있게 검출된다. 반대로 공중 오검출 쪽은 등속 하강이
원리적으로 1g를 넘지 않으므로 남는 위험은 프롭 진동뿐이고, 100~300Hz 진동은
5Hz LPF에서 거의 완전히 소멸한다.

주의: 이 표는 v=0.4m/s를 가정한다. `FS_DESCENT_DELTA_US`를 벤치에서 올려
하강률이 커지면 스파이크는 v²로 커지므로 검출은 더 쉬워진다. 반대로 하강률을
낮추면 여유가 줄어드니 α와 임계를 같이 재확인해야 한다.
